...
export const AlgoNode: NsGraph.INodeRender = props => {
  return (
    <div className={`xflow-algo-node ${props.isNodeTreePanel ? 'panel-node' : ''} ${'node-'+ props.data.label}`} >
      <span className="icon">
        <DatabaseOutlined style={fontStyle} />
      </span>
      <span className={`${'label-'+ props.data.label} flex-label`} >{props.data.label}</span>
